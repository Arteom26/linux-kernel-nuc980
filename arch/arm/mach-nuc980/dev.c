...
//#include <linux/i2c/i2c-hid.h> //schung
...
static struct gpiod_lookup_table nuc980_i2c_gpiod_table = {
        ...
        .table = {
		/* NUC980_PB7 */
                GPIO_LOOKUP_IDX("nuc980_gpio_port", 39, NULL, 0,
                                GPIO_ACTIVE_HIGH | GPIO_OPEN_DRAIN),
		/* NUC980_PB5 */
                GPIO_LOOKUP_IDX("nuc980_gpio_port", 37, NULL, 1,
                                GPIO_ACTIVE_HIGH | GPIO_OPEN_DRAIN),
        },
};
...
int nuc980_gpio_core_direction_in(struct gpio_chip *gc,
                unsigned gpio_num);
int nuc980_gpio_core_get(struct gpio_chip *gc, unsigned gpio_num);
void nuc980_gpio_core_set(struct gpio_chip *gc, unsigned gpio_num,
                                 int val);
int nuc980_gpio_core_direction_out(struct gpio_chip *gc,
                unsigned gpio_num, int val);
void __init nuc980_platform_init(struct platform_device **device, int size)
{
#if defined(CONFIG_GPIO_NUC980) || defined(CONFIG_GPIO_NUC980_MODULE)
#if defined(CONFIG_I2C_ALGOBIT) || defined(CONFIG_I2C_ALGOBIT_MODULE)
	gpiod_add_lookup_table(&nuc980_i2c_gpiod_table);
#endif
#endif	
