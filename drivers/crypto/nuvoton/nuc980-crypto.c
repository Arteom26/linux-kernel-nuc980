...
static int nuc980_crypto_probe(struct platform_device *pdev)
{
    ...
    nuc980_crdev.crypto_clk = of_clk_get(pdev->dev.of_node, 0);
	if (IS_ERR(nuc980_crdev.crypto_clk)) {
		pr_err("%s clk_get error!!\n", __func__);
		return -ENODEV;
	}

	/* Enable Cryptographic Accerlator clock */
    clk_prepare_enable(nuc980_crdev.crypto_clk);
...
static int nuc980_crypto_remove(struct platform_device *pdev)
{
	...
	clk_disable(nuc980_crdev.crypto_clk);
...
{
	clk_enable(nuc980_crdev.crypto_clk);
	return 0;
}
