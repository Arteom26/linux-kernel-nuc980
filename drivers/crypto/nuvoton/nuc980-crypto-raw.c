...
static int  ECC_VerifySignature(void)
{
	volatile struct nuc980_crypto_regs  *crpt_regs = nuc980_crdev.regs;
...
static long nvt_ecc_ioctl(struct file *filp, unsigned int cmd, unsigned long arg)
{
	...
	case ECC_IOC_SET_PRI_KEY:
		...
		return copy_from_user(ecc_d, (char *)arg, pCurve->Echar);

	case ECC_IOC_SET_PUB_K1:
		memset(ecc_x1, 0, sizeof(ecc_x1));
		return copy_from_user(ecc_x1, (char *)arg, pCurve->Echar);

	case ECC_IOC_SET_PUB_K2:
		memset(ecc_y1, 0, sizeof(ecc_y1));
		return copy_from_user(ecc_y1, (char *)arg, pCurve->Echar);

	case ECC_IOC_SET_SCALAR_K:
		memset(ecc_k, 0, sizeof(ecc_k));
		return copy_from_user(ecc_k, (char *)arg, pCurve->Echar);

	case ECC_IOC_SET_MSG:
		memset(ecc_msg, 0, sizeof(ecc_msg));
		return copy_from_user(ecc_msg, (char *)arg, pCurve->Echar);

	case ECC_IOC_SET_SIG_R:
		memset(ecc_r, 0, sizeof(ecc_r));
		return copy_from_user(ecc_r, (char *)arg, pCurve->Echar);

	case ECC_IOC_SET_SIG_S:
		memset(ecc_s, 0, sizeof(ecc_s));
		return copy_from_user(ecc_s, (char *)arg, pCurve->Echar);

	case ECC_IOC_GET_PUB_K1:
		return copy_to_user((char *)arg, ecc_x1, pCurve->Echar);

	case ECC_IOC_GET_PUB_K2:
		return copy_to_user((char *)arg, ecc_y1, pCurve->Echar);

	case ECC_IOC_GET_SIG_R:
		return copy_to_user((char *)arg, ecc_r, pCurve->Echar);

	case ECC_IOC_GET_SIG_S:
		return copy_to_user((char *)arg, ecc_s, pCurve->Echar);
...
/**
  ...
  * @return  -1    Error
  */
static int  RSA_GenerateSignature(int rsa_len, char *n, char *d, char *C, char *msg, char *sig)
{
	volatile struct nuc980_crypto_regs  *crpt_regs = nuc980_crdev.regs;
	int  i;
...
/**
  ...
  */
static int RSA_VerifySignature(int rsa_len, char *n, char *e, char *C, char *sig, char *msg)
{
	volatile struct nuc980_crypto_regs  *crpt_regs = nuc980_crdev.regs;
...
static long nvt_rsa_ioctl(struct file *filp, unsigned int cmd, unsigned long arg)
{
	...
	case RSA_IOC_SET_N:
		...
		return copy_from_user(g_rsa_N, (char *)arg, g_rsa_len/4);

	case RSA_IOC_SET_E:
		memset(g_rsa_E, 0, sizeof(g_rsa_E));
		return copy_from_user(g_rsa_E, (char *)arg, g_rsa_len/4);

	case RSA_IOC_SET_D:
		memset(g_rsa_d, 0, sizeof(g_rsa_d));
		return copy_from_user(g_rsa_d, (char *)arg, g_rsa_len/4);

	case RSA_IOC_SET_C:
		memset(g_rsa_C, 0, sizeof(g_rsa_C));
		return copy_from_user(g_rsa_C, (char *)arg, g_rsa_len/4);

	case RSA_IOC_SET_MSG:
		memset(g_rsa_Msg, 0, sizeof(g_rsa_Msg));
		return copy_from_user(g_rsa_Msg, (char *)arg, g_rsa_len/4);

	case RSA_IOC_SET_SIG:
		memset(g_rsa_Sig, 0, sizeof(g_rsa_Sig));
		return copy_from_user(g_rsa_Sig, (char *)arg, g_rsa_len/4);

	case RSA_IOC_GET_MSG:
		return copy_to_user((char *)arg, g_rsa_Msg, g_rsa_len/4);

	case RSA_IOC_GET_SIG:
		return copy_to_user((char *)arg, g_rsa_Sig, g_rsa_len/4);
