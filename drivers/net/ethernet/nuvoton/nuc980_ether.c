...
#define REG_WKUPSER1    (0x054)  /*!< System Wakeup Source Enable Register1 */
#define REG_WKUPSSR0    (0x058)  /*!< System Wakeup Source Status Register0 */
#define REG_WKUPSSR1    (0x05C)  /*!< System Wakeup Source Status Register1 */

#define DRV_MODULE_NAME		"nuc980-emc"
...
#define REG_TSCTL 		0x100
#define REG_TSSEC 		0x110 /* [31:0] */
#define REG_TSSUBSEC 	0x114 /* [30:0] */
#define REG_TSINC 		0x118
#define REG_TSADDEND 	0x11c
#define REG_UPDSEC 		0x120
#define REG_UPDSUBSEC 	0x124
#define REG_ALMSEC 		0x128
#define REG_ALMSUBSEC 	0x11c
...
#define ETH_TRIGGER_RX(base)		do{__raw_writel(ENSTART, base + REG_RSDR);}while(0)
#define ETH_TRIGGER_TX(base)		do{__raw_writel(ENSTART, base + REG_TSDR);}while(0)
#define ETH_ENABLE_TX(base)			do{__raw_writel(__raw_readl(base + REG_MCMDR) | MCMDR_TXON, base + REG_MCMDR);}while(0)
#define ETH_ENABLE_RX(base)			do{__raw_writel(__raw_readl(base + REG_MCMDR) | MCMDR_RXON, base + REG_MCMDR);}while(0)
#define ETH_DISABLE_TX(base)		do{__raw_writel(__raw_readl(base + REG_MCMDR) & ~MCMDR_TXON, base + REG_MCMDR);}while(0)
#define ETH_DISABLE_RX(base)		do{__raw_writel(__raw_readl(base + REG_MCMDR) & ~MCMDR_RXON, base + REG_MCMDR);}while(0)
...
static void nuc980_config_hw_tstamping(void __iomem *base, u32 data)
{
	__raw_writel(data, base + REG_TSCTL);
}

static int nuc980_config_addend(void __iomem *base, u32 addend)
{
	int limit;

	__raw_writel(addend, base + REG_TSADDEND);
	
	limit = 10;
	while (limit--) {
		if ((__raw_readl(base + REG_TSADDEND) == addend))
			break;
		mdelay(10);
	}
...
static int nuc980_init_systime(struct nuc980_ether *priv, u32 sec, u32 nsec)
{
	...
	__raw_writel(ns >> 31, priv->reg_base + REG_UPDSEC);
	__raw_writel(ns & 0x7FFFFFFF, priv->reg_base + REG_UPDSUBSEC);
	
	/* issue command to initialize the system time value */
	value = __raw_readl(priv->reg_base + REG_TSCTL);
	value |= TSIEN;
	__raw_writel(value, priv->reg_base + REG_TSCTL);

	/* wait for present system time initialize to complete */
	limit = 10;
	while (limit--) {
		if (!(__raw_readl(priv->reg_base + REG_TSCTL) & TSIEN))
			break;
		mdelay(10);
	}
	__raw_readl(priv->reg_base + REG_TSSUBSEC); // read subsec to update sec
...
{
	struct nuc980_ether *priv = container_of(ptp, struct nuc980_ether, ptp_clock_ops);
	unsigned long flags;
	u32 diff, addend;
	int neg_adj = 0;
	u64 adj;
	...
	adj = addend * scaled_ppm;
	adj >>= 16;
	diff = div_u64(adj, 1000000ULL);
	addend = neg_adj ? (addend - diff) : (addend + diff);
	
	spin_lock_irqsave(&priv->ptp_lock, flags);
	nuc980_config_addend(priv->reg_base, addend);
...
static int nuc980_adjust_time(struct ptp_clock_info *ptp, s64 delta)
{
	...
	__raw_writel(sec, priv->reg_base + REG_UPDSEC);
	__raw_writel(nsec, priv->reg_base + REG_UPDSUBSEC);

	__raw_writel(__raw_readl(priv->reg_base + REG_TSCTL)|TSUPDATE, priv->reg_base + REG_TSCTL);

	limit = 10;
	while (limit--) {
		if (!(__raw_readl(priv->reg_base + REG_TSCTL) & TSUPDATE))
			break;
		mdelay(10);
	}
	__raw_readl(priv->reg_base + REG_TSSUBSEC); // read subsec to update sec
...
static int nuc980_get_time(struct ptp_clock_info *ptp, struct timespec64 *ts)
{
	...
	sec = __raw_readl(priv->reg_base + REG_TSSEC);
	subsec = __raw_readl(priv->reg_base + REG_TSSUBSEC);
...
static void nuc980_ptp_unregister(struct nuc980_ether *priv)
{
	if (priv->ptp_clock) {
		...
		//netdev_info(priv->netdev, "unregistered PTP clock\n");
	}
}

static int nuc980_init_ptp(struct nuc980_ether *priv)
{
	nuc980_config_hw_tstamping(priv->reg_base, 0);//stop HW timestamp
	priv->hwts_tx_en = 0;
	priv->hwts_rx_en = 0;
...
static void nuc980_get_tx_hwtstamp(struct nuc980_ether *priv, struct nuc980_txbd *txbd, struct sk_buff *skb)
{
	...
	ns = nuc980_desc_get_timestamp(priv, (void *)txbd, 1);
...
static void nuc980_opmode(struct net_device *netdev, void __iomem *base, int speed, int duplex)
{
	...
	val = __raw_readl(base + REG_MCMDR);
	...
	__raw_writel(val, base + REG_MCMDR);

	ETH_TRIGGER_TX(base); // in case some packets queued in descriptor
}
...
static void adjust_link(struct net_device *netdev)
{
	...
	if (status_change) {
		...
		nuc980_opmode(netdev, ether->reg_base, ether->speed, ether->duplex);
	}
}
				...
				unsigned int x, const unsigned char *pval)
{
	...
	__raw_writel(lsw, ether->reg_base + REG_CAML_BASE + x * CAM_ENTRY_SIZE);
	__raw_writel(msw, ether->reg_base + REG_CAMM_BASE + x * CAM_ENTRY_SIZE);
}
...
static void nuc980_set_fifo_threshold(struct net_device *netdev)
{
	...
	val = TXTHD | BLENGTH;
	__raw_writel(val, ether->reg_base + REG_FFTCR);
}
...
static void nuc980_return_default_idle(struct net_device *netdev)
{
	...
	val = __raw_readl(ether->reg_base + REG_MCMDR);
	val |= SWR;
	__raw_writel(val, ether->reg_base + REG_MCMDR);
}
...
//		ENCRCE   |   /* CRC Error Interrupt Enable Bit */
	...
	__raw_writel(val, ether->reg_base + REG_MIEN);
}
...
{
	struct nuc980_ether *ether = netdev_priv(netdev);
	*val = __raw_readl(ether->reg_base + REG_MISTA) & mask;
	__raw_writel(*val, ether->reg_base + REG_MISTA);
}
...
static void nuc980_set_global_maccmd(struct net_device *netdev)
{
	...
	__raw_writel(MAX_PACKET_SIZE, ether->reg_base + REG_DMARFC);
	__raw_writel(val, ether->reg_base + REG_MCMDR);
}
...
static void nuc980_enable_cam(struct net_device *netdev)
{
	...
	val = __raw_readl(ether->reg_base + REG_CAMEN);
	val |= CAM0EN;
	__raw_writel(val, ether->reg_base + REG_CAMEN);
}
...
static void nuc980_enable_cam_command(struct net_device *netdev)
{
	...
	__raw_writel(val, ether->reg_base + REG_CAMCMR);
}
...
static void nuc980_set_curdest(struct net_device *netdev)
{
	...
	__raw_writel(ether->start_rx_ptr, ether->reg_base + REG_RXDLSA);
	__raw_writel(ether->start_tx_ptr, ether->reg_base + REG_TXDLSA);
}
...
static void nuc980_reset_mac(struct net_device *netdev)
{
	...
	ETH_DISABLE_TX(ether->reg_base);
	ETH_DISABLE_RX(ether->reg_base);
    ...
    ETH_ENABLE_TX(ether->reg_base);

	if (netif_queue_stopped(netdev))
		netif_wake_queue(netdev);
}

static int nuc980_mdio_write(struct mii_bus *bus, int phy_id, int regnum, u16 value)
{
	struct nuc980_ether *ether = bus->priv;
	unsigned long timeout = jiffies + msecs_to_jiffies(MII_TIMEOUT * 100);

	__raw_writel(value, ether->reg_base + REG_MIID);
	__raw_writel((phy_id << 0x08) | regnum | PHYBUSY | MDCON | PHYWR, ether->reg_base + REG_MIIDA);


	/* Wait for completion */
	while (__raw_readl(ether->reg_base + REG_MIIDA) & PHYBUSY) {
...
static int nuc980_mdio_read(struct mii_bus *bus, int phy_id, int regnum)
{
	...
	__raw_writel((phy_id << 0x08) | regnum | PHYBUSY | MDCON, ether->reg_base + REG_MIIDA);
	...
	return __raw_readl(ether->reg_base + REG_MIID);
}
...
static int nuc980_set_mac_address(struct net_device *netdev, void *addr)
{
	...
	memcpy((void*)netdev->dev_addr, address->sa_data, netdev->addr_len);
	nuc980_write_cam(netdev, CAM0, netdev->dev_addr);
...
static int nuc980_poll(struct napi_struct *napi, int budget)
{
	...
	if(complete) {
		napi_complete(napi);
		__raw_writel(__raw_readl(ether->reg_base + REG_MIEN) | ENRXINTR, ether->reg_base + REG_MIEN);
	}

rx_out:

	ETH_TRIGGER_RX(ether->reg_base);
	return(rx_cnt);
}
...
static int nuc980_ether_open(struct net_device *netdev)
{
	...
	if (nuc980_init_ptp(ether))
		netdev_warn(netdev, "PTP init failed\n");
...
static void nuc980_ether_set_multicast_list(struct net_device *netdev)
{
	...
	__raw_writel(rx_mode, ether->reg_base + REG_CAMCMR);
}
...
static int nuc980_get_eee(struct net_device *netdev, struct ethtool_keee *edata)
{
	return -EOPNOTSUPP;
}

static int nuc980_set_eee(struct net_device *netdev, struct ethtool_keee *edata)
{
	return -EOPNOTSUPP;
}
...
static void nuc980_get_regs(struct net_device *netdev, struct ethtool_regs *regs, void *p)
{
	...
	regs->version = 0;
	memcpy(p, ether->reg_base + REG_CAMCMR, 76 * sizeof(u32));
}
...
static int nuc980_ether_probe(struct platform_device *pdev)
{
	...
	ether->reg_base = ioremap(ether->res->start, resource_size(ether->res));
	if (!ether->reg_base) {
	...
	ether->clk = of_clk_get(pdev->dev.of_node, 0);
	if (IS_ERR(ether->clk)) {
		dev_err(&pdev->dev, "failed to get emac clock\n");
		error = PTR_ERR(ether->clk);
		goto err1;
	}	
	clk_prepare_enable(ether->clk);

	struct clk *div_clk = clk_get_parent(ether->clk);
	if (IS_ERR(div_clk)) {
		dev_err(&pdev->dev, "failed to get divider clock clock\n");
		error = PTR_ERR(div_clk);
		goto err1;
	}	
	clk_set_rate(div_clk, 1000000);
...
static int nuc980_ether_suspend(struct platform_device *pdev, pm_message_t state)
{
	...
	if (netif_running(netdev)) {
		...
		if(ether->wol) {  // enable wakeup from magic packet
			__raw_writel(__raw_readl(ether->reg_base + REG_MCMDR) | MCMDR_MGPWAKE, ether->reg_base + REG_MCMDR);
			__raw_writel(__raw_readl(ether->reg_base + REG_WKUPSER1) | (1 << 16), ether->reg_base + REG_WKUPSER1);
		} else {
			if (ether->phy_dev)
				phy_stop(ether->phy_dev);
			else if (ether->use_ncsi)
...
static const struct of_device_id nuc980_emac_of_match[] = {
	{ .compatible = "nuvoton,nuc980-emac" },
	{},
};
MODULE_DEVICE_TABLE(of, nuc980_emac_of_match);
...
static struct platform_driver nuc980_ether_driver = {
	...
	.driver		= {
		.name	= "nuc980-emac",
		.of_match_table = of_match_ptr(nuc980_emac_of_match),
		.owner	= THIS_MODULE,
	},
};
