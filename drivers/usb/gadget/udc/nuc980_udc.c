...
static void paser_usb_irq(struct nuc980_udc *udc, int irq)
{
...
static void paser_irq_cep(struct nuc980_udc *udc, u32 irq)
{
...
static void paser_irq_nep(struct nuc980_ep *ep, u32 irq)
{
	struct nuc980_udc *udc = ep->dev;
	struct nuc980_request *req;
	unsigned int volatile timeout;
...
//0-3F for Ctrl pipe
static s32 get_sram_base(u32 idx, u32 max)
{
	u32 i;
	s32 sram_addr;
...
static int nuc980_udc_probe(struct platform_device *pdev)
{
	...
	udc->clk = of_clk_get(pdev->dev.of_node, 0);
	if (IS_ERR(udc->clk))
	{
		error = -ENODEV;
