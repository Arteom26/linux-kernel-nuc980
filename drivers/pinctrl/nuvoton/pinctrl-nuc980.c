...
#define REG_PDID		(0x000)	/* Product Identifier Register */
#define REG_PWRON		(0x004)	/* Power-On Setting Register */
#define REG_ARBCON		(0x008)	/* Arbitration Control Register */
#define REG_LVRDCR 		(0x020)	/* Low Voltage Reset & Detect Control Register */
#define REG_MISCFCR  	(0x030)	/* Miscellaneous Function Control Register */
#define REG_MISCIER 	(0x040)	/* Miscellaneous Interrupt Enable Register */
#define REG_MISCISR 	(0x044)	/* Miscellaneous Interrupt Status Register */
#define REG_ROMSUM0		(0x048)	/* Internal ROM BIST Checksum Register 0 */
#define REG_ROMSUM1		(0x04C)	/* Internal ROM BIST Checksum Register 1 */
...
#define REG_AHBIPRST	(0x060)	/* AHB IP Reset Control Register */
#define REG_APBIPRST0	(0x064)	/* APB IP Reset Control Register 0 */
#define REG_APBIPRST1	(0x068)	/* APB IP Reset Control Register 1 */
#define REG_RSTSTS		(0x06C)	/* Reset Source Active Status Register */
 ...
 */
static int nuc980_pmx_set_mux(struct pinctrl_dev *pctldev, unsigned selector, unsigned group)
{
	struct nuc980_pinctrl *info = pinctrl_dev_get_drvdata(pctldev);
	...
	if (!capable(CAP_SYS_RAWIO)) {
		printk(KERN_ERR "Driver does not have necessary capabilities for MMIO operations.\n");
		return -EPERM;  // or handle appropriately
	}
...
static int nuc980_pinconf_get(struct pinctrl_dev *pctldev, unsigned pin_id, unsigned long *config)
{
	return 0;
}

static int nuc980_pinconf_set(struct pinctrl_dev *pctldev, unsigned pin_id, unsigned long *configs, unsigned num_configs)
{
	return 0;
}
...
static int nuc980_pinctrl_parse_functions(struct device_node *np, struct nuc980_pinctrl *info, u32 index)
{
	struct device_node *child;
...
static int nuc980_pinctrl_probe(struct platform_device *pdev)
{
	...
	if (ret < 0) {
		dev_err(&pdev->dev, "Resource fail for GPIO banks\n");
		return ret;
	}
	...
	info->functions = devm_kzalloc(&pdev->dev, info->nfunctions * sizeof(struct nuc980_pmx_func), GFP_KERNEL);
	if (!info->functions) {
		return -ENOMEM;
	}

	info->groups = devm_kzalloc(&pdev->dev, info->ngroups * sizeof(struct nuc980_pin_group), GFP_KERNEL);
	if (!info->groups) {
		return -ENOMEM;
	}
	...
	info->pctldev = devm_pinctrl_register(&pdev->dev, &nuc980_pinctrl_desc, info);
