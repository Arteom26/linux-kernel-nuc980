...
#define	NUC980_SPI0_PHYS_BASE_ADDR 0xB0061000
...
struct nuc980_spi {
	...
	struct nuc980_dma_done  spi_dma_slave_done;
	volatile int spi_slave_done_state;
	struct wait_queue_head spi_slave_done;
};
static DECLARE_WAIT_QUEUE_HEAD(spi0_slave_done);
static DECLARE_WAIT_QUEUE_HEAD(spi1_slave_done);

static void spi_nuc980_slave_dma_callback(void *arg)
{
	struct nuc980_spi *spi_dev = arg;

	spi_dev->spi_dma_slave_done.done = true;
	spi_dev->spi_slave_done_state = 1;
	wake_up_interruptible((spi_dev->phys_addr == NUC980_SPI0_PHYS_BASE_ADDR) ? &spi0_slave_done : &spi1_slave_done);
	return;
}

static inline struct nuc980_spi *to_hw(struct spi_device *sdev)
{
...
static inline void nuc980_spi_chipsel(struct spi_device *spi, int value)
{
	switch (value) {
...
static inline void nuc980_spi_setup_txbitlen(struct nuc980_spi *hw,
...
static int nuc980_spi_txrx(struct spi_device *spi, struct spi_transfer *t)
{
	struct nuc980_spi *hw = (struct nuc980_spi *)to_hw(spi);
	unsigned long end;
	...
	if(hw->using_pdma){
		...
		if (t->rx_buf) {
			...
			pdma->slave_config.src_addr = hw->phys_addr + REG_RX;
			if (!hw->is_spidev && !(t->len % 4) && !(((int)t->rx_buf) % 4)) {
			...
			hw->spi_dma_slave_done.done = false;
			pdma->rxdesc->callback = spi_nuc980_slave_dma_callback;
		...
		pdma->slave_config.dst_addr = hw->phys_addr + REG_TX;
		if (!hw->is_spidev && !(t->len % 4) && !(((int)t->tx_buf) % 4)) {
			__raw_writel((__raw_readl(hw->regs + REG_CTL)&~(0x1F00))|(0x80000), hw->regs + REG_CTL);//32 bits,byte reorder
		...
		if (!t->rx_buf) {
			pdma->txdesc->callback = spi_nuc980_slave_dma_callback;
		...
		if(hw->phys_addr == NUC980_SPI0_PHYS_BASE_ADDR)
			wait_event_interruptible(spi0_slave_done, (hw->spi_slave_done_state != 0));
		else
			wait_event_interruptible(spi1_slave_done, (hw->spi_slave_done_state != 0));
		hw->spi_slave_done_state = 0;
	...
	else { // DMA transfers disabled
		// TX first in case waiting for response from master/slave
		bool busWidth4Bytes = false;
		uint32_t length = t->len;
		if (!hw->is_spidev && !(t->len % 4) && !(((int)t->tx_buf) % 4)) {
			__raw_writel((__raw_readl(hw->regs + REG_CTL)&~(0x1F00))|(0x80000), hw->regs + REG_CTL);//32 bits,byte reorder
			busWidth4Bytes = true;
			length = t->len/4;
		...
		if (t->tx_buf) {
			...
			for(int i = 0;i < t->len; i++) {
				(busWidth4Bytes) ? __raw_writel(*((uint32_t*)(t->tx_buf) + i), hw->regs + REG_TX) : __raw_writel(*((uint8_t*)(t->tx_buf) + i), hw->regs + REG_TX);
				while(__raw_readl(hw->regs + REG_STATUS) & 0x10000); // Wait for TX buffer to empty

				// Check for timeout
				if (time_after(jiffies, end)) {
					dev_warn(hw->dev, "SPI TX wait busy timeout: %d\n", __LINE__);
					return 0;
				}
			}
		}
		...
		if(t->rx_buf) {
			...
			for(int i = 0;i < t->len; i++) {
				while(__raw_readl(hw->regs + REG_STATUS) & 0x100); // Wait for RX Buffer to contain data
				if(busWidth4Bytes)
					*(((uint32_t*)t->rx_buf) + i) = __raw_readl(hw->regs + REG_RX);
				else
					*(((uint8_t*)t->rx_buf) + i) = __raw_readl(hw->regs + REG_RX);

				// Check for timeout
				if (time_after(jiffies, end)) {
					dev_warn(hw->dev, "SPI RX wait busy timeout: %d\n", __LINE__);
					return 0;
				}
			}
		}
...
static int nuc980_spi_update_state(struct spi_device *spi,
...
static int nuc980_spi_setupxfer(struct spi_device *spi,
                                 struct spi_transfer *t)
{
	struct nuc980_spi *hw = (struct nuc980_spi *)to_hw(spi);
	int ret;

	ret = nuc980_spi_update_state(spi, t);
	if (ret)
		return ret;

	nuc980_set_divider(hw);
	nuc980_spi_setup_txbitlen(hw, hw->pdata->txbitlen);
...
static int nuc980_spi_setup(struct spi_device *spi)
{
	struct nuc980_spi *hw = (struct nuc980_spi *)to_hw(spi);
	int ret;

	ret = nuc980_spi_update_state(spi, NULL);
	if (ret)
		return ret;
...
static struct nuc980_spi_info *nuc980_spi_parse_dt(struct device *dev)
{
...
static int nuc980_spi_probe(struct platform_device *pdev)
{
	...
	hw->master = spi_controller_get(master);
	hw->pdata = nuc980_spi_parse_dt(&pdev->dev);
	pdma = &hw->dma;
	hw->dev = &pdev->dev;
	hw->spi_slave_done_state = 0;
	hw->is_spidev = 0;
	...
	u32 temp[2];
	if (of_property_read_u32(pdev->dev.of_node, "pdma-enable", temp)) {
		dev_warn(&pdev->dev, "can't get pdma-enable from dt\n");
		hw->using_pdma = false;
	} else {
		hw->using_pdma = temp[0];
	}

	if (of_property_read_u32_array(pdev->dev.of_node, "pdma-lines", temp, 2)) {
		dev_warn(&pdev->dev, "can't get pdma-lines from dt\n");
		hw->using_pdma = false;
	}
	else {
		pdma->tx_line = temp[0];
	...
	if (hw->using_pdma) {
		...
		*/
		...
		hw->spi_slave_done = spi0_slave_done;
	}
	...
	hw->bitbang.ctlr           = hw->master;
	hw->bitbang.setup_transfer = nuc980_spi_setupxfer;
	hw->bitbang.chipselect     = nuc980_spi_chipsel;
	hw->bitbang.txrx_bufs      = nuc980_spi_txrx;
...
err_irq:
	iounmap(hw->regs);
err_pdata:
	spi_controller_put(hw->master);

err_nomem:
	return err;
}

static int nuc980_spi_remove(struct platform_device *dev)
{
	struct nuc980_spi *hw = platform_get_drvdata(dev);
	struct nuc980_ip_dma *pdma = &hw->dma;

	if (hw->using_pdma && pdma->chan_rx)
		dma_release_channel(pdma->chan_rx);
	if (hw->using_pdma && pdma->chan_tx)
...
#ifdef CONFIG_PM
static int nuc980_spi_suspend(struct device *dev)
{
...
static int nuc980_spi_resume(struct device *dev)
{
	struct nuc980_spi *hw = dev_get_drvdata(dev);
...
static const struct dev_pm_ops nuc980_spi_pmops = {
	.suspend    = nuc980_spi_suspend,
	.resume     = nuc980_spi_resume,
};

#define NUC980_SPI_PMOPS (&nuc980_spi_pmops)

#else
#define NUC980_SPI_PMOPS NULL
#endif

static const struct of_device_id nuc980_spi_of_match[] = {
	{   .compatible = "nuvoton,nuc980-spi" },
	{	},
};
MODULE_DEVICE_TABLE(of, nuc980_spi_of_match);

static struct platform_driver nuc980_spi_driver = {
	.probe      = nuc980_spi_probe,
	.remove     = nuc980_spi_remove,
	.driver     = {
		.name   = "nuc980-spi",
		.owner  = THIS_MODULE,
		.pm	= NUC980_SPI_PMOPS,
		.of_match_table = of_match_ptr(nuc980_spi_of_match),
	},
};
module_platform_driver(nuc980_spi_driver);

MODULE_DESCRIPTION("nuc980 spi driver!");
MODULE_LICENSE("GPL");
MODULE_ALIAS("platform:nuc980-spi");
